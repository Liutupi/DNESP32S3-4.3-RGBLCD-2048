...
esp_err_t gt9xxx_init(void)
{
    ...
    i2c_device_config_t gt9xxx_i2c_dev_conf = {
        ...
        .device_address  = 0x14,                    /* 先尝试 0x14 地址 */
    };
    /* I2C总线上添加gt9xxx设备 */
    ESP_ERROR_CHECK(i2c_master_bus_add_device(myiic1_bus_handle, &gt9xxx_i2c_dev_conf, &gt9xxx_handle));

    /* 尝试读取产品ID */
    esp_err_t err = gt9xxx_rd_reg(GT9XXX_PID_REG, temp, 4);
    if (err != ESP_OK)
    {
        /* 如果 0x14 失败，则尝试 0x5D 地址 */
        i2c_master_bus_rm_device(gt9xxx_handle);
        gt9xxx_i2c_dev_conf.device_address = 0x5D;
        ESP_ERROR_CHECK(i2c_master_bus_add_device(myiic1_bus_handle, &gt9xxx_i2c_dev_conf, &gt9xxx_handle));
        err = gt9xxx_rd_reg(GT9XXX_PID_REG, temp, 4);
    }
    
    if (err != ESP_OK)
    {
        sprintf(touch_debug_info, "I2C FAIL err=%d", err);
        ESP_LOGE("GT9XXX", "Failed to communicate with touch IC at both 0x14 and 0x5D");
        return 1;
    }

    temp[4] = 0;
    sprintf(touch_debug_info, "PID: %s", temp);
    /* 判断一下是否是特定的触摸屏 */
    if (strcmp((char *)temp, "911") && strcmp((char *)temp, "9147") && strcmp((char *)temp, "1158") && strcmp((char *)temp, "9271") && strcmp((char *)temp, "928"))
    {
        sprintf(touch_debug_info, "Invalid PID: %s", temp);
    ...
    /* 读取配置版本(0x8047) 和 分辨率(0x8146) */
    uint8_t cfg_ver = 0;
    uint8_t res_buf[4] = {0};
    uint8_t cfg_head[8] = {0};
    gt9xxx_rd_reg(GT9XXX_CFGS_REG, &cfg_ver, 1);
    gt9xxx_rd_reg(0x8146, res_buf, 4);
    gt9xxx_rd_reg(0x8047, cfg_head, 8);
    uint16_t x_res = res_buf[0] | (res_buf[1] << 8);
    uint16_t y_res = res_buf[2] | (res_buf[3] << 8);

    sprintf(touch_debug_info, "CFG: %02X %02X %02X %02X %02X %02X %02X %02X", 
            cfg_head[0], cfg_head[1], cfg_head[2], cfg_head[3],
            cfg_head[4], cfg_head[5], cfg_head[6], cfg_head[7]);
...
uint8_t gt9xxx_scan(uint8_t mode)
{
    ...
    if (mode & 0x80)
    {
        /* 若触摸点数量超出最大支持点数，也强制清空状态以防死锁 */
        if ((mode & 0XF) > g_gt_tnum || (mode & 0XF) == 0)
        {
            i = 0;
        ...
        else
        {
            ...
            sprintf(touch_debug_info, "T:%d X:%d Y:%d", (mode & 0xF), tp_dev.x[0], tp_dev.y[0]);
    ...
    {
        /* 没有触摸时持续刷新状态，以防死区 */
        sprintf(touch_debug_info, "Wait... Mode: %02X", mode);
    }
