...
static bool write_pcm_frame(uint8_t *pcm, uint32_t decoded_size, const esp_audio_dec_info_t *info)
{
    if (!pcm || decoded_size == 0) {
        return true;
    }
    if (info && info->sample_rate > 0 && info->sample_rate != board_audio_i2s_sample_rate()) {
        if (board_audio_i2s_start((int)info->sample_rate) != ESP_OK) {
            ESP_LOGE(TAG, "RADIO_URL_FAILED reason=I2S reinit failed sample_rate=%u",
    ...
    if (channels == 1) {
        if (decoded_size * 2 > RADIO_PCM_BUFFER_SIZE) {
            ESP_LOGE(TAG, "RADIO_URL_FAILED reason=mono PCM frame too large");
            return false;
        }
    ...
    return board_audio_i2s_write(pcm16, samples);
}
...
static bool play_one_url(player_task_arg_t *arg, const char *url, int url_index)
{
    ...
    if (!client) {
        ESP_LOGW(TAG, "RADIO_URL_FAILED reason=http_client_init url=%s", url);
        return false;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "RADIO_URL_FAILED reason=http_open err=%s url=%s", esp_err_to_name(err), url);
        goto cleanup;
    }
    ...
    if (status < 200 || status >= 400) {
        ESP_LOGW(TAG, "RADIO_URL_FAILED reason=http_status status=%d url=%s", status, url);
        goto cleanup;
    }
    if (content_type_is_rejected(content_type) || !content_type_is_mp3(content_type)) {
        ESP_LOGW(TAG, "RADIO_URL_FAILED reason=content_type content-type=%s url=%s",
    ...
    if (dec_err != ESP_AUDIO_ERR_OK) {
        ESP_LOGW(TAG, "RADIO_URL_FAILED reason=decoder_open err=%d url=%s", (int)dec_err, url);
        goto cleanup;
    }
    ...
    while (!s_stop_requested) {
        ...
        while (raw_len > 0 && !s_stop_requested) {
            ...
            if (!write_pcm_frame(pcm, out.decoded_size, &info)) {
...
static void player_task(void *param)
{
    ...
    if (!played && !s_stop_requested) {
        emit_status(arg, RADIO_PLAYER_FAILED, "RADIO_URL_FAILED all_urls_failed");
    }
